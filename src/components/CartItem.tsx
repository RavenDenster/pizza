...
const CartItemBlock: React.FC<CartItemProps> = ({ id, title, type, size, price, count , imageUrl }) => {
    ...
    const onClickPlus = () => {
      //ignore
        dispatch(addItem({
            id, // только id так как по логики у нас в карзине не может создаться новый объект, а долько увеличивать count
        }as CartItem)
      )
    }
    const onClickMinus = () => {
  ...
  return (
    <div className="cart__item">
        ...
        <div className="cart__item-count">
          <button disabled={count === 1} onClick={onClickMinus} className={clsx("button button--outline button--circle cart__item-count-minus", {
